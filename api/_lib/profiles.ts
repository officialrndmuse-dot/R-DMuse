...
// Admin-only: every registered customer, enriched with order aggregates.
// Aggregated in application code rather than a SQL join, since
// orders.user_id has no declared FK relationship for PostgREST embedding,
// and the data volume here is small (a boutique store, not enterprise scale).
